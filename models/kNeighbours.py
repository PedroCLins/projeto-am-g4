...
def tuned_k_neighbors(X_train: np.ndarray, y_train: pd.Series) -> KNeighborsClassifier:
    ...
    for n_neighbors, weights, metric, leaf_size in product(params['n_neighbors'], params['weights'], params['metric'], params['leaf_size']):
        ...
        metrics_scores.append({
            'params': (n_neighbors, weights, metric, leaf_size),
    ...
    best_model = KNeighborsClassifier(n_neighbors=best_overall_params['params'][0],
                                      ...
                                      metric=best_overall_params['params'][2],
                                      leaf_size=best_overall_params['params'][3])
